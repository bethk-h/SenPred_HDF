...
```{r}
hdfseurat_filt <- readRDS("../Output/Seurat_Objects/hdfseuratfiltered_allcells.rds")

 hdfseurat_filt_3D <- subset(hdfseurat_filt, Sample == "DS3D" | Sample == "EP3D")
#hdfseurat_filt_3D <- subset(hdfseurat_filt, Sample == "DS3D")
```
...
write.csv(hdfseurat.markers,"../Output/HDF_Data_Analysis/Cluster_markers/HDF3D_clustermarkers_posandneg.csv")
```
...
##Exploring principal  components in the training dataset
```{r}
#Exploring principal components in the referencedataset 
pca <- training[['pca']]
head(Loadings(training, reduction = "pca"))
output <- as.matrix( Loadings(training, reduction = "pca"))
loadings <- as.matrix(pca@feature.loadings)


PCA <- (ProjectDim(training, reduction = "pca"))
VizDimLoadings(PCA, dims = 1:10, reduction = "pca")
DimHeatmap(PCA, dims = 1, cells = 500, balanced = TRUE)
PCA <- as.matrix(PCA@reductions[["pca"]]@feature.loadings)

embeddings <- as.matrix(pca@cell.embeddings)
barcodelabels <- training@meta.data[c("Barcode","Sample")]


merge <- merge(embeddings, barcodelabels, by='row.names')
DS3D <- subset(merge, Sample == "DS3D")
DS3D <- DS3D[-c(1,52, 53)]
EP3D <- subset(merge, Sample == "EP3D")
EP3D <- EP3D[-c(1,52, 53)]

medianEP3D <- as.matrix(apply(EP3D,2,median))
medianDS3D <- as.matrix(apply(DS3D,2,median))

merge2 <- merge(medianEP3D, medianDS3D, by='row.names')
rowname <- gsub("PC", "scpred", merge2$Row.names)
merge3 <- cbind (merge2, rowname)

merge4 <- merge3[-c(1)]

colnames(merge4) <- c("EP3D_3Dmodel", "DS3D_3Dmodel", "PC")
rownames(merge4) <- merge4[,3]
merge4 <- merge4[,-3]
merge4 <- as.matrix(merge4)

install.packages("heatmap3")
library("gplots")

breaks <- unique(c(seq(-2,-1.1,length=100),seq(-1,0.9,length=100), seq(1,2,length=100)))
my_palette <- colorRampPalette(c("yellow","white","white", "blue"))(length(breaks)-1)


heatmap.2(merge4, scale = "none", col = bluered(100), 
          trace = "none", density.info = "none", cexRow=0.3)



heatmap(merge4, cexRow=0.3, cexCol=0.7,  breaks=seq(-2, -1, 0,1,2))
